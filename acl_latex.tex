...
\textit{Base64 encoding is a detector failure, not a defense result.} It flags 26.7\% of clean documents while catching only 32.8\% of blatant override payloads---worse on the easy case than no defense at all. Its negative CDG reflects a detector that has stopped discriminating, not camouflage becoming visible. Without clean controls it would have looked like the only defense that worked, which is why we urge that detector-side defense evaluations always report a false-positive rate.

\paragraph{What this evaluation does and does not establish.}
Spotlighting and structured queries were designed as \textbf{agent-side prevention}: they exist to stop an agent acting on injected instructions, not to help a detector notice them. Applying them detector-side is a fair reading of the question---the detector is an LLM asked to find an injection in a document, and these transforms mark which region is untrusted---but it is not their native setting, and our results should not be read as showing that these defenses fail at what they were built to do. Whether they reduce \textit{attack success rate} agent-side against camouflaged payloads is a distinct and unanswered question, and the natural extension of this work. Our structured-query condition is additionally an inference-time approximation: the published method fine-tunes on the instruction/data separation, which we do not reproduce, and we label it an approximation throughout.
...
We introduced domain-camouflaged injection---payloads carrying their malicious instruction as an assertive domain claim rather than a directive to the agent---and the Camouflage Detection Gap (CDG) to measure it. Across 45 tasks and two model families, detectors calibrated on static payloads exhibit a large, statistically significant blind spot (CDG = 0.896 for Llama 3.1 8B, 0.444 for Gemini 2.0 Flash), concentrated in HIGH-confidence errors no threshold can recover. These are upper bounds: under a context-blind attacker CDG is 0.778, still large. The gap survives every dedicated injection defense we tested. Deployments using smaller, locally-hosted detectors therefore face a vulnerability that neither exemplar augmentation nor current spotlighting and structured-query defenses address. We release our framework, task bank, and payload corpus to support follow-on work.
