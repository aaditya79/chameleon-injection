...
Our camouflage generator, however, is given \textbf{the full text of the target document} at generation time. We make this deliberate mismatch explicit rather than eliding it: the full-context generator is a \textit{worst-case upper bound} on camouflage quality, not a model of the typical attacker. It answers ``how large can this gap get?'' rather than ``how large is it usually?'' Every CDG figure in this paper's main experiments should be read as an upper bound, and we label it as such throughout. We additionally run the stated, weaker attacker as its own condition below, so the paper reports a bounded range rather than a single optimistic point.
...
The amplification is of similar magnitude for both payload types (2.40 vs 2.68). An earlier version reported a large asymmetry---3.4$\times$ static against 9.9$\times$ camouflage---and attributed it to camouflage interacting with debate conformity dynamics. That asymmetry does not survive a matched baseline; it was an artifact of dividing a 45-task debate ASR by a single-agent ASR drawn from five financial tasks, 21 of whose 56 trials were also invalid records. What survives is the coarser claim: debate helps on the stronger model and hurts on the weaker one, for camouflaged and static payloads alike.
...
Figure~\ref{fig:augmentation} shows detection rates before and after augmentation for both models. For Llama, the augmented detector improves camouflage IDR from 11.9\% to 20.7\%---a 75\% relative gain, still leaving 79.3\% of injections undetected. Static detection is essentially unchanged (100.0\% vs 99.9\%).
...
We report this explicitly rather than silently revising the number, because the tenfold claim was prominent and a reader who encountered it should know what happened to it. The surviving claim is coarser but rests on matched arms: debate degrades injection robustness on the weaker model and improves it on the stronger one, for camouflaged and static payloads alike, and the conformity pressure it induces is measurable even where aggregate outcomes improve.

%% ============================================================
\section{Conclusion}
%% ============================================================

We introduced domain-camouflaged injection---payloads that carry their malicious instruction as an assertive domain claim rather than as a directive to the agent---and the Camouflage Detection Gap (CDG) as a diagnostic metric for detector robustness against it. Across 45 tasks and two model families, detectors calibrated on static payloads exhibit a large, statistically significant blind spot (CDG = 0.896 for Llama 3.1 8B, CDG = 0.444 for Gemini 2.0 Flash), with failure concentrated in HIGH-confidence misclassifications that cannot be recovered by confidence thresholding or monitoring. Because our generator is given the full target document, these are upper bounds on the gap rather than estimates of the typical case. Targeted detector augmentation with one camouflaged exemplar per domain nearly closes the gap for Gemini 2.0 Flash but not for Llama 3.1 8B; we report this as a limit of exemplar augmentation at that scale rather than as a general property of few-shot detection. Our findings suggest that deployments using smaller, locally-hosted detectors face an injection detection vulnerability that prompt-level exemplar fixes do not address, though establishing whether dedicated injection defenses close it remains open. We release our framework, task bank, and payload corpus to support follow-on work.
